...
이 문서는 여러 Feature와 공통 오류 처리에서 사용하는 앱 전역 Dialog 요청·결과·queue 계약을
정의한다. UI 상태 책임은 [`feature-ui.md`](feature-ui.md), 공용 Dialog UI는
[`design-system.md`](design-system.md)를 함께 따른다.

## 1. 단일 진입점

Feature 또는 공통 오류 처리 코드에서 앱 전역 Dialog가 필요하면 `core:common`의
`showGlobalDialog(...)`만 직접 호출한다.

```text
Feature 또는 공통 오류 처리
    → core:common showGlobalDialog(...)
    → core:common SharedFlow
    → app GlobalDialogManager와 GlobalDialogHost
    → designsystem DMinusDialog
    → GlobalDialogResult
```
...
`showGlobalDialog(...)`는 사용자 선택 또는 overflow 결과가 결정될 때까지 suspend된다.

```kotlin
val result = showGlobalDialog(
    GlobalDialogRequest(
        title = "작업을 종료할까요?",
...
State 변경 또는 Effect 발행으로 연결한다. Dialog UI나 Manager를 직접 참조하지 않는다.

## 4. `dismissible`

- `true`면 Back과 Dialog 바깥 영역 터치를 허용하고 `Dismiss`를 반환한다.
...
- process 종료 후 요청, 결과 대기와 Dialog UI는 영속 복원하지 않는다.

## 6. 모듈별 책임

| 모듈 | 책임 |
|---|---|
| `feature:*` | Dialog 필요 여부 결정, 호출 결과에 따른 후속 동작 |
| `core:common` | 플랫폼 독립 요청·결과, 단일 SharedFlow와 호출 함수 |
| `app` | process lifetime 수집, FIFO·overflow·취소 처리, 최상단 렌더링 |
| `designsystem` | stateless `DMinusDialog` UI와 callback |
| `catalog` | 제품 runtime 없이 Dialog 주요 시각 상태를 Story로 노출 |

## 7. 금지 사항

- 호출자가 `globalDialogEvents`, `GlobalDialogManager` 또는 `GlobalDialogHost`를 직접 참조
- Feature가 전역 Dialog를 위해 `DMinusDialog`를 직접 렌더링
- Request model에 color, spacing, shape, typography 같은 표현 정보 추가
- Request model에 호출자 callback 추가
- Dialog title과 message를 log, analytics 또는 crash report에 기록
