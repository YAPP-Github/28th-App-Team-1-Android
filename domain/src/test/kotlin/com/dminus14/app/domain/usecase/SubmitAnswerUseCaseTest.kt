...
class SubmitAnswerUseCaseTest {
    ...
    fun `답변 제출을 성공하면 결과를 반환한다`() =
        runTest {
            ...
            val repository = FakeInterviewRepository(submitResult = expected)
            ...
            assertEquals(1L, repository.requestedQuestionId)
            assertEquals(true, repository.requestedIsWrapUp)
        }

    @Test
    fun `답변 제출 중 예외가 발생하면 실패 결과를 반환한다`() =
        runTest {
            val exception = RuntimeException("답변 제출 실패")
    ...
    private open class FakeInterviewRepository(
        private val submitResult: SubmitAnswerResult =
            SubmitAnswerResult(
                answerId = null,
                nextQuestion = null,
                sessionEnded = true,
        ...
        var requestedSessionId: Long? = null
            private set
        var requestedQuestionId: Long? = null
            private set
        var requestedIsWrapUp: Boolean? = null
            private set
