...
class IssueVideoUploadUrlUseCaseTest {
    @Test
    fun `비디오 업로드 URL 발급을 성공하면 결과를 반환한다`() =
        runTest {
            val expected =
            ...
            val repository = FakeInterviewRepository(uploadUrlResult = expected)
            val useCase = IssueVideoUploadUrlUseCase(repository)

            val result = useCase(42L)

            assertTrue(result.isSuccess)
            assertSame(expected, result.getOrThrow())
            assertEquals(1, repository.issueUploadUrlCallCount)
            assertEquals(42L, repository.requestedSessionId)
        }

    @Test
    fun `비디오 업로드 URL 발급 중 예외가 발생하면 실패 결과를 반환한다`() =
        runTest {
            val exception = RuntimeException("URL 발급 실패")
    ...
    private open class FakeInterviewRepository(
        private val uploadUrlResult: InterviewVideoUploadUrl =
            InterviewVideoUploadUrl(
                uploadUrl = "",
                contentType = "",
                expiresInSeconds = 0L,
