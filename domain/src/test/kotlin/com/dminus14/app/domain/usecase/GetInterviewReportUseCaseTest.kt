...
class GetInterviewReportUseCaseTest {
    @Test
    fun `면접 레포트 상세 조회를 성공하면 결과를 반환한다`() =
        runTest {
            val expected =
                InterviewReport(
            ...
            val repository = FakeInterviewRepository(reportResult = expected)
            val useCase = GetInterviewReportUseCase(repository)
    ...
    @Test
    fun `면접 레포트 상세 조회 중 예외가 발생하면 실패 결과를 반환한다`() =
        runTest {
            val exception = RuntimeException("레포트 상세 조회 실패")
    ...
    private open class FakeInterviewRepository(
        private val reportResult: InterviewReport =
            InterviewReport(
                status = InterviewReportStatus.READY,
                headline = null,
                video = null,
